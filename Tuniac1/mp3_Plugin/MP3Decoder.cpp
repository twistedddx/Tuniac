...
bool CMP3Decoder::GetXingHeader(unsigned char * XingBuffer)
{
...
bool CMP3Decoder::GetStreamData(void)
{
	...
	m_pHelper->LogConsoleMessage(TEXT("MP3 Decoder"), TEXT("Error calculating mp3 stream information."));

	return false;
}

bool CMP3Decoder::Open(LPTSTR szFilename)
{
	m_LastLayer = -1;
...
bool		CMP3Decoder::GetFormat(unsigned long * SampleRate, unsigned long * Channels)
{
...
bool		CMP3Decoder::SetState(unsigned long State)
{
	return true;
}
...
bool		CMP3Decoder::GetLength(unsigned long * MS)
{
	*MS = m_StreamLengthMS;
	return true;
}

bool		CMP3Decoder::SetPosition(unsigned long * MS)
{
