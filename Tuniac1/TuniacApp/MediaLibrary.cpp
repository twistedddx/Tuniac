...
bool CMediaLibrary::LoadMediaLibrary(LPTSTR szLibraryFolder)
{
	...
	else if (MLDH.Version != TUNIAC_MEDIALIBRARY_VERSION && MLDH.Version != TUNIAC_MEDIALIBRARY_VERSION09 && MLDH.Version != TUNIAC_MEDIALIBRARY_VERSION08 && MLDH.Version != TUNIAC_MEDIALIBRARY_VERSION07 && MLDH.Version != TUNIAC_MEDIALIBRARY_VERSION06 && MLDH.Version != TUNIAC_MEDIALIBRARY_VERSION05)
	{
		//if (tuniacApp.m_LogWindow)
		//{
		//	if (tuniacApp.m_LogWindow->GetLogOn())
	...
	else
	{
		...
		for(unsigned long x = 0; x < MLDH.NumEntries; x++)
		{
			...
			if (MLDH.Version == TUNIAC_MEDIALIBRARY_VERSION09)
			{
				LibraryEntry09 MLE;
				ReadFile(hLibraryFile, &MLE, sizeof(LibraryEntry09), &BytesRead, NULL);
				if (BytesRead != sizeof(LibraryEntry09))
				{
					//if (tuniacApp.m_LogWindow)
					//{
					//	if (tuniacApp.m_LogWindow->GetLogOn())
					//tuniacApp.m_LogWindow->LogMessage(TEXT("MediaLibrary"), TEXT("MediaLibrary is corrupt, resetting library."));
					//}
					MessageBox(NULL, TEXT("MediaLibrary 0.9 is corrupt, resetting library."), TEXT("Startup Error"), MB_OK | MB_ICONWARNING);
					m_MediaLibrary.RemoveAll();
					bOK = false;
					break;
				}
				...
				StringCchCopy(libraryEntry.szFileType, 16, MLE.szFileType);
			...
			else if (MLDH.Version == TUNIAC_MEDIALIBRARY_VERSION08)
			{
				LibraryEntry08 MLE;
