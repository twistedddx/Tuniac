...
bool COGGDecoder::Open(LPTSTR szSource)
{
...
bool		COGGDecoder::GetFormat(unsigned long * SampleRate, unsigned long * Channels)
{
	vorbis_info *	vi = ov_info(&m_vf, -1);
...
bool		COGGDecoder::GetLength(unsigned long * MS)
{
...
bool		COGGDecoder::SetPosition(unsigned long * MS)
{
...
bool		COGGDecoder::SetState(unsigned long State)
{
	return(true);
}

bool		COGGDecoder::GetBuffer(float ** ppBuffer, unsigned long * NumSamples)
{
	float ** pcm;
	vorbis_info *	vi = ov_info(&m_vf, -1);
...
//		so we need to REORDER
	...
	else
	{
		...
		for(unsigned long x=0; x<bytes_read; x++)
		{
			for(unsigned long channel=0; channel < vi->channels; channel++)
			{
