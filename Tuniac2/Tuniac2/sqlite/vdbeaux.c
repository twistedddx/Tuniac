...
static char *displayP3(Op *pOp, char *zTemp, int nTemp){
  ...
  switch( pOp->p3type ){
    case P3_KEYINFO: {
      ...
      sprintf_s(zTemp, nTemp, "keyinfo(%d", pKeyInfo->nField);
      i = strlen(zTemp);
      for(j=0; j<pKeyInfo->nField; j++){
        ...
        if( pColl ){
          ...
          strcpy_s(&zTemp[i], nTemp, pColl->zName);
          i += n;
        }else if( i+4<nTemp-6 ){
          strcpy_s(&zTemp[i],nTemp,",nil");
          i += 4;
        }
      }
      zTemp[i++] = ')';
    ...
    case P3_COLLSEQ: {
      ...
      sprintf_s(zTemp, nTemp, "collseq(%.20s)", pColl->zName);
      zP3 = zTemp;
      break;
    }
    case P3_FUNCDEF: {
      ...
      char zNum[30];
      sprintf_s(zTemp, nTemp, "%.*s", nTemp, pDef->zName);
      sprintf_s(zNum, 30, "(%d)", pDef->nArg);
      if( strlen(zTemp)+strlen(zNum)+1<=nTemp ){
        strcat_s(zTemp, nTemp, zNum);
      }
      zP3 = zTemp;
      break;
    }
