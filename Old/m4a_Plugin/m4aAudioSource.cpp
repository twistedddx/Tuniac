...
Cm4aAudioSource::Cm4aAudioSource(void)
{
	m_hDecoder		= NULL;
...
bool Cm4aAudioSource::Open(LPTSTR szSource)
{
...
bool		Cm4aAudioSource::GetLength(unsigned long * MS)
{
    long length;
...
bool		Cm4aAudioSource::SetPosition(unsigned long * MS)
{
	int64_t		duration;
	float		fms = *MS;
...
bool		Cm4aAudioSource::SetState(unsigned long State)
{
	return true;
}

bool		Cm4aAudioSource::GetFormat(unsigned long * SampleRate, unsigned long * Channels)
{
...
bool		Cm4aAudioSource::GetBuffer(float ** ppBuffer, unsigned long * NumSamples)
{
